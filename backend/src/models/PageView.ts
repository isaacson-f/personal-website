...
export class PageView extends BaseModel {
   ...
   */
  private static get schema(): Joi.ObjectSchema<PageViewData> {
    return Joi.object({
   ...
   */
  static async create(pageViewData: Partial<PageViewData>): Promise<PageViewData> {
    // Validate input data
    const validation = this.validate<PageViewData>(this.schema, pageViewData);
   ...
   */
  static async findById(id: string): Promise<PageViewData | null> {
    if (!id) return null;
   ...
   */
  static async findBySession(sessionId: string, options: { limit?: number } = {}): Promise<PageViewData[]> {
    if (!sessionId) return [];
   ...
   */
  static async update(id: string, updates: Partial<Pick<PageViewData, 'scroll_depth' | 'time_on_page' | 'exit_page'>>): Promise<PageViewData> {
    if (!id) throw new Error('Page view ID is required');

    // Validate updates
    const allowedFields = ['scroll_depth', 'time_on_page', 'exit_page'] as const;
   ...
   */
  static async markAsExitPage(id: string): Promise<PageViewData> {
    if (!id) throw new Error('Page view ID is required');
   ...
   */
  static async getStatistics(filters: FilterOptions & { url?: string } = {}): Promise<{
   ...
   * Get popular pages
   */
  static async getPopularPages(filters: FilterOptions & { limit?: number } = {}): Promise<Array<{
    url: string;
    page_views: number;
