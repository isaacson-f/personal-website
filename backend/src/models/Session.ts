...
export class Session extends BaseModel {
   ...
   */
  private static get schema(): Joi.ObjectSchema<SessionData> {
    return Joi.object({
      id: Joi.string().max(255).required(),
      visitor_id: Joi.string().max(255).allow(null),
   ...
   */
  static async create(sessionData: Partial<SessionData>): Promise<SessionData> {
    // Validate input data
    const validation = this.validate<SessionData>(this.schema, sessionData);
   ...
   * Find session by ID
   */
  static async findById(id: string): Promise<SessionData | null> {
    if (!id) return null;
   ...
   */
  static async findActiveByVisitor(visitorId: string): Promise<SessionData[]> {
    if (!visitorId) return [];
   ...
   */
  static async findByVisitor(visitorId: string, options: { limit?: number } = {}): Promise<SessionData[]> {
    if (!visitorId) return [];
   ...
   * Update session data
   */
  static async update(id: string, updates: Partial<Pick<SessionData, 'end_time' | 'page_views' | 'duration_seconds' | 'device_info' | 'geographic_data'>>): Promise<SessionData> {
   ...
   * End a session
   */
  static async endSession(id: string, endTime: Date | null = null): Promise<SessionData> {
    if (!id) throw new Error('Session ID is required');
   ...
   */
  static async incrementPageViews(id: string, count: number = 1): Promise<SessionData> {
    if (!id) throw new Error('Session ID is required');
   ...
   */
  static async endExpiredSessions(timeoutMinutes: number = 30): Promise<number> {
